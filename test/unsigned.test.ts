...
test("uint8", (t) =>
	t.is(Buffer.from(unsigned.encode(uint8)).toString("hex"), "00"))

test("uint8 max length", (t) => t.is(unsigned.encodingLength(uint8max), 2))
test("uint8 max", (t) =>
	t.is(Buffer.from(unsigned.encode(uint8max)).toString("hex"), "ff01"))

test("uint16 length", (t) => t.is(unsigned.encodingLength(uint16), 2))
test("uint16", (t) =>
	t.is(Buffer.from(unsigned.encode(uint16)).toString("hex"), "8002"))

test("uint16 max length", (t) => t.is(unsigned.encodingLength(uint16max), 3))
test("uint16 max", (t) =>
	t.is(Buffer.from(unsigned.encode(uint16max)).toString("hex"), "ffff03"))

test("uint32 length", (t) => t.is(unsigned.encodingLength(uint32), 3))
test("uint32", (t) =>
	t.is(Buffer.from(unsigned.encode(uint32)).toString("hex"), "808004"))

test("uint32 max length", (t) => t.is(unsigned.encodingLength(uint32max), 5))
test("uint32 max", (t) =>
	t.is(Buffer.from(unsigned.encode(uint32max)).toString("hex"), "ffffffff0f"))

test("uint64 length", (t) => t.is(unsigned.encodingLength(uint64), 5))
test("uint64", (t) =>
	t.is(Buffer.from(unsigned.encode(uint64)).toString("hex"), "8080808010"))

test("uint64 max length", (t) => t.is(unsigned.encodingLength(uint64max), 10))
test("uint64 max", (t) =>
	t.is(
		Buffer.from(unsigned.encode(uint64max)).toString("hex"),
...
test("uint128", (t) =>
	t.is(
		Buffer.from(unsigned.encode(uint128)).toString("hex"),
...
for (let l = 1; l < 15; l++) {
	...
	test(`byte ${l + 1} min length`, (t) =>
		t.is(unsigned.encodingLength(min), l + 1))
	test(`byte ${l + 1} min`, (t) =>
